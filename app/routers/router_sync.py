...
async def pull_script(key: str = Query(default="")):
    ...
    lines += [
        ...
        "",
        "# Reportar sesiones activas a Railway para monitoreo en tiempo real",
        ":local nac_sessions \"\"",
        ":local nac_sep \"\"",
        ":foreach s in=[/ip hotspot active find] do={",
        "  :local nac_u [/ip hotspot active get $s user]",
        "  :local nac_a [/ip hotspot active get $s address]",
        "  :local nac_m [/ip hotspot active get $s mac-address]",
        "  :local nac_bi [/ip hotspot active get $s bytes-in]",
        "  :local nac_bo [/ip hotspot active get $s bytes-out]",
        "  :local nac_up [/ip hotspot active get $s uptime]",
        "  :set nac_sessions ($nac_sessions . $nac_sep . \"{\\\"user\\\":\\\"\" . $nac_u . \"\\\",\\\"address\\\":\\\"\" . $nac_a . \"\\\",\\\"mac_address\\\":\\\"\" . $nac_m . \"\\\",\\\"bytes_in\\\":\" . $nac_bi . \",\\\"bytes_out\\\":\" . $nac_bo . \",\\\"uptime\\\":\\\"\" . $nac_up . \"\\\"}\")",
        "  :set nac_sep \",\"",
        "}",
        ":if ($nac_sessions != \"\") do={",
        f'  :do {{ /tool fetch url="https://nac-production.up.railway.app/api/traffic/report" http-method=post http-header-field="Content-Type:application/json,X-Sync-Key:{SYNC_KEY}" http-data=(\"{{\\\"sessions\\\":[\" . $nac_sessions . \"]}}\") mode=https output=none }} on-error={{}}',
        "}",
        "",
