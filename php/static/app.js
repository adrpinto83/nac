...
function navigateTo(page) {
    ...
    if (page === 'reports')      loadReportsInit();
}

// ── Reportes de consumo por fecha ──
...
async function loadReportsInit() {
    ...
    // Poblar selector de empleados (una sola vez)
    const sel = document.getElementById('repUser');
        ...
        try {
            ...
            users.filter(u => u.approval_status === 'approved')
                 ...
                 .forEach(u => {
                     ...
                     o.value = u.id;
                     o.textContent = `${u.full_name} (${u.username})` + (u.role === 'admin' ? ' · admin' : '');
                     sel.appendChild(o);
                 });
        } catch (e) { /* sin selector si falla */ }
    }
    loadReport();
}
...
async function loadReport() {
    ...
    const uid  = document.getElementById('repUser').value;
    const tbody = document.getElementById('reportTbody');
    const single = !!uid;
    document.getElementById('repCol1').textContent = single ? 'Fecha' : 'Empleado';
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--text-muted)">Generando…</td></tr>';
    try {
        let url = `${API}/traffic/report-range?from=${from}&to=${to}`;
        if (uid) url += `&user_id=${uid}`;
        const data = await fetchAPI(url);
        _reportData = data;

        document.getElementById('repTotDown').textContent = fmtBytes(data.totals.bytes_down);
        document.getElementById('repTotUp').textContent   = fmtBytes(data.totals.bytes_up);
        document.getElementById('repTotAll').textContent  = fmtBytes(data.totals.bytes_total);

        // Gráfico
        const chartCard = document.getElementById('reportChartCard');
        document.getElementById('repChartTitle').textContent =
            single ? 'Consumo diario' : 'Consumo por empleado (top 15)';
        const chartHtml = reportChartHTML(data);
        document.getElementById('reportChart').innerHTML = chartHtml || '<p style="color:var(--text-muted);text-align:center">Sin datos para graficar</p>';
        chartCard.style.display = chartHtml ? '' : 'none';

        if (single) {
            // Vista por empleado: desglose diario
            const u = data.users[0];
            document.getElementById('repTableTitle').textContent =
                u ? `Desglose diario — ${u.full_name}` : 'Desglose diario';
            if (!data.daily.length) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--text-muted)">Sin consumo en este rango</td></tr>';
                return;
            }
            tbody.innerHTML = data.daily.map(d => `
                <tr>
                    <td style="font-weight:600">${fmtDate(d.date)}</td>
                    <td style="color:var(--text-muted)">—</td>
                    <td style="text-align:right">—</td>
                    <td style="text-align:right;color:#3b82f6;font-weight:500">${fmtBytes(d.bytes_down)}</td>
                    <td style="text-align:right;color:#10b981;font-weight:500">${fmtBytes(d.bytes_up)}</td>
                    <td style="text-align:right;font-weight:600">${fmtBytes(d.bytes_total)}</td>
                </tr>`).join('');
        } else {
            document.getElementById('repTableTitle').textContent = 'Consumo por empleado';
            if (!data.users.length) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--text-muted)">Sin consumo en este rango</td></tr>';
                return;
            }
            tbody.innerHTML = data.users.map(r => `
                <tr>
                    <td>
                        <div style="font-weight:600">${escHtml(r.full_name)}${r.role === 'admin' ? ' <span style="font-size:.7rem;color:#7c3aed">admin</span>' : ''}</div>
                        <div style="font-size:.78rem;color:var(--text-muted)">${escHtml(r.username)}</div>
                    </td>
                    <td style="color:var(--text-muted)">${escHtml(r.department || '—')}</td>
                    <td style="text-align:right;color:var(--text-muted)">${r.sessions}</td>
...
// Construye un gráfico de barras horizontales (estilos inline → sirve en pantalla y en el PDF)
function reportChartHTML(data) {
    let items;
    if (data.user_id) {
        items = (data.daily || []).map(d => ({ label: fmtDate(d.date), down: d.bytes_down, up: d.bytes_up, total: d.bytes_total }));
    } else {
        items = (data.users || []).slice(0, 15).map(u => ({ label: u.full_name || u.username, down: u.bytes_down, up: u.bytes_up, total: u.bytes_total }));
    }
    if (!items.length) return '';
    const max = Math.max(1, ...items.map(i => i.total));
    const rows = items.map(i => {
        const dPct = (i.down / max * 100).toFixed(1);
        const uPct = (i.up / max * 100).toFixed(1);
        return `
        <div style="display:flex;align-items:center;gap:8px;margin:6px 0;font-size:12px">
            <div style="width:140px;flex:0 0 140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#374151">${escHtml(i.label)}</div>
            <div style="flex:1;background:#f1f5f9;border-radius:4px;height:18px;display:flex;overflow:hidden">
                <div title="Descarga" style="width:${dPct}%;background:#3b82f6;height:100%"></div>
                <div title="Subida" style="width:${uPct}%;background:#10b981;height:100%"></div>
            </div>
            <div style="width:90px;flex:0 0 90px;text-align:right;font-weight:600;color:#111">${fmtBytes(i.total)}</div>
        </div>`;
    }).join('');
    const legend = `<div style="display:flex;gap:16px;font-size:11px;color:#6b7280;margin-bottom:8px">
        <span><span style="display:inline-block;width:10px;height:10px;background:#3b82f6;border-radius:2px"></span> Descarga</span>
        <span><span style="display:inline-block;width:10px;height:10px;background:#10b981;border-radius:2px"></span> Subida</span></div>`;
    return legend + rows;
}

function exportReportPDF() {
    if (!_reportData) { toast('Genera un reporte primero', 'error'); return; }
    const d = _reportData;
    const single = !!d.user_id;
    const titleSubject = single && d.users[0] ? d.users[0].full_name : 'Todos los empleados';

    let tableHead, tableRows;
    if (single) {
        tableHead = '<th>Fecha</th><th class="r">Descarga</th><th class="r">Subida</th><th class="r">Total</th>';
        tableRows = (d.daily || []).map(x => `<tr><td>${fmtDate(x.date)}</td><td class="r">${fmtBytes(x.bytes_down)}</td><td class="r">${fmtBytes(x.bytes_up)}</td><td class="r b">${fmtBytes(x.bytes_total)}</td></tr>`).join('');
    } else {
        tableHead = '<th>Empleado</th><th>Departamento</th><th class="r">Sesiones</th><th class="r">Descarga</th><th class="r">Subida</th><th class="r">Total</th>';
        tableRows = (d.users || []).map(x => `<tr><td>${escHtml(x.full_name)}<br><small>${escHtml(x.username)}</small></td><td>${escHtml(x.department || '—')}</td><td class="r">${x.sessions}</td><td class="r">${fmtBytes(x.bytes_down)}</td><td class="r">${fmtBytes(x.bytes_up)}</td><td class="r b">${fmtBytes(x.bytes_total)}</td></tr>`).join('');
    }

    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Reporte de consumo</title>
    <style>
      ...
      .card{flex:1;border:1px solid #e5e7eb;border-radius:8px;padding:10px;text-align:center}
      .card .lbl{font-size:11px;color:#6b7280} .card .val{font-size:18px;font-weight:700}
      table{width:100%;border-collapse:collapse;margin-top:12px;font-size:12px}
      th,td{border-bottom:1px solid #e5e7eb;padding:6px 8px;text-align:left}
      th{background:#f8fafc} .r{text-align:right} .b{font-weight:700}
      small{color:#6b7280} .sec{font-weight:600;margin:18px 0 6px;font-size:13px}
      @media print{.noprint{display:none}}
      .btn{padding:8px 16px;background:#4f46e5;color:#fff;border:none;border-radius:6px;cursor:pointer}
    </style></head><body>
    <button class="btn noprint" onclick="window.print()">🖨️ Imprimir / Guardar PDF</button>
    <h1>Reporte de consumo</h1>
    <div class="sub">${escHtml(titleSubject)} &nbsp;·&nbsp; ${d.from} a ${d.to} &nbsp;·&nbsp; generado ${new Date().toLocaleString('es')}</div>
    <div class="cards">
      <div class="card"><div class="lbl">Descarga total</div><div class="val" style="color:#3b82f6">${fmtBytes(d.totals.bytes_down)}</div></div>
      <div class="card"><div class="lbl">Subida total</div><div class="val" style="color:#10b981">${fmtBytes(d.totals.bytes_up)}</div></div>
      <div class="card"><div class="lbl">Total general</div><div class="val">${fmtBytes(d.totals.bytes_total)}</div></div>
    </div>
    <div class="sec">${single ? 'Consumo diario' : 'Consumo por empleado'}</div>
    ${reportChartHTML(d)}
    <table><thead><tr>${tableHead}</tr></thead><tbody>${tableRows || '<tr><td colspan="6">Sin datos</td></tr>'}</tbody></table>
...
function exportReportCSV() {
    ...
    if (d.user_id) {
        const u = d.users[0];
        header = ['Fecha', 'Descarga (bytes)', 'Subida (bytes)', 'Total (bytes)'];
        rows = d.daily.map(x => [x.date, x.bytes_down, x.bytes_up, x.bytes_total]);
        name = `reporte_${(u ? u.username : 'empleado')}_${d.from}_a_${d.to}.csv`;
    } else {
        header = ['Empleado', 'Usuario', 'Departamento', 'Sesiones', 'Descarga (bytes)', 'Subida (bytes)', 'Total (bytes)'];
        rows = d.users.map(x => [x.full_name, x.username, x.department || '', x.sessions, x.bytes_down, x.bytes_up, x.bytes_total]);
        name = `reporte_consumo_${d.from}_a_${d.to}.csv`;
    }
...
function accessActions(u) {
    ...
    btns.push(`<button class="btn btn-primary btn-sm" onclick="openEditUserModal(${u.id})" title="Editar usuario">✎</button>`);
    btns.push(`<button class="btn btn-danger btn-sm" onclick="deleteUser(${u.id},'${escAttr(u.full_name)}')" title="Eliminar usuario">🗑</button>`);
    return btns.join(' ');
}
        ...
        : list.map(d => `
            <tr${d.approval_status === 'pending' ? ' style="background:var(--warning-bg,#fffbeb)"' : ''}>
                ...
                <td style="white-space:nowrap">
                    ...
                    `}
                    <button class="btn btn-primary btn-sm" onclick="openEditDeviceModal(${d.id})" title="Editar / Reasignar">✎</button>
                </td>
            </tr>`).join('');
}
...
function openEditUserModal(userId) {
    ...
    if (!u) return;
    document.getElementById('editUserLabel').textContent = u.full_name || u.username;
    ...
    document.getElementById('editUserPos').value       = u.position     || '';
...
async function saveEditUser() {
    const body = {
        full_name:  document.getElementById('editUserFullName').value.trim() || null,
        email:      document.getElementById('editUserEmail').value.trim()    || null,
        phone:      document.getElementById('editUserPhone').value.trim()    || null,
        department: document.getElementById('editUserDept').value.trim()     || null,
        position:   document.getElementById('editUserPos').value.trim()      || null,
    };
    try {
        await fetchAPI(`${API}/users/${_editUserId}`, { method: 'PUT', body: JSON.stringify(body) });
...
async function deleteUser(userId, name) {
    if (!confirm(`¿Eliminar al usuario "${name}"?\nSe eliminarán también sus dispositivos. Esta acción no se puede deshacer.`)) return;
    try {
        await fetchAPI(`${API}/users/${userId}`, { method: 'DELETE' });
        toast(`🗑 Usuario "${name}" eliminado.`, 'ok');
        loadUsers();
        loadDashboard();
    } catch (e) { toast(`❌ Error: ${e.message}`, 'err'); }
}

// ── Editar / Reasignar dispositivo ───────────────────────────────────────────
let _editDeviceId = null;

async function openEditDeviceModal(deviceId) {
    _editDeviceId = deviceId;
    const d = allDevices.find(d => d.id === deviceId);
    if (!d) return;
    document.getElementById('editDevMacLabel').textContent = d.mac_address;
    document.getElementById('editDevType').value = d.device_type || 'unknown';
    document.getElementById('editDevOs').value   = d.os_type || '';
    const sel = document.getElementById('editDevUser');
...
async function saveEditDevice() {
    const userId = document.getElementById('editDevUser').value;
    const body = {
        user_id:     userId ? parseInt(userId, 10) : null,
        device_type: document.getElementById('editDevType').value,
        os_type:     document.getElementById('editDevOs').value.trim() || null,
    };
    try {
        await fetchAPI(`${API}/devices/${_editDeviceId}`, { method: 'PUT', body: JSON.stringify(body) });
